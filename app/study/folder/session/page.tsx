...
function FolderSessionInner() {
  ...
  // This page ONLY reviews graduated cards; learning happens on the folder's ladder/pathway.
  const category: StudyCategory | null =
    categoryParam === 'graduated' || categoryParam === 'due' || categoryParam === 'dormant' || categoryParam === 'starred'
      ? categoryParam : null

  // The legacy step-pipeline learning flow is gone: a bare URL (the old "new + due" folder session)
  // and the removed new/learning categories land on the folder's ladder/pathway instead.
  const legacyLearningUrl = !category
  useEffect(() => {
    if (legacyLearningUrl) {
      router.replace(routes.ladderFolder(folderId,
        categoryParam === 'new' || categoryParam === 'learning' ? { category: categoryParam } : {}))
    }
  ...
  useEffect(() => {
    async function load() {
      ...
      // One wave for everything that needs only the user id — the overrides, scheduler-params and
      // confusion-link fetches used to run as separate serial stages around this Promise.all.
      const [existingOverrides, allDecks, allFolders, pipeline, thisFolder, profileData, allParamRows, intraLinksAll] = await Promise.all([
      ...
      // Primary-pair params come from the bulk listForUser row when it exists (the common case) —
      // the old getOrCreate here cost 2 extra serial round trips. Only a brand-new pair awaits it.
      const firstDeck = decks[0]
      if (firstDeck) {
        try {
          const paramsRow =
            allParamRows.find(r => r.sourceLanguage === firstDeck.sourceLanguage && r.targetLanguage === firstDeck.targetLanguage && r.answerField === 'forward_typed')
      ...
      // Second wave: the folder's cards, states and deck prefs in THREE bulk queries instead of
      // 2-3 per deck (the old per-deck for…of serialized round trips across the whole folder).
      // Cards come back WITHOUT audio blobs (SESSION_CARD_COLUMNS); queued clips hydrate below.
      const folderDeckIds = decks.map(d => d.id)
      const [cardsByDeck, allStatesList] = await Promise.all([
        cardRepo.listForDecks(folderDeckIds),
  ...
  const handleAnswer = useCallback(async (rating: Rating, wasCorrect: boolean, userAnswer = '', _issueType?: GradingIssueType, softWrongRecallRating?: Rating) => {
    ...
    try {
      ...
      if (newState.graduated && newState.relearningStep > 0) {
        const requeued: SessionCard = { ...current, state: newState, productionMode: decideProductionMode(newState, nowDate, Math.random, schedulerParams), relearnLapsedAt: reviewCountRef.current }
