...
function LibraryPageBody({ pairSource: initPairSource, pairTarget: initPairTarget }: { pairSource: string | null; pairTarget: string | null }) {
  ...
  function getAllPairs(): LanguagePair[] {
    ...
    for (const d of allDecks) {
      ...
      if (!seen.has(key)) {
        seen.add(key)
        result.push({ id: key, ownerId: userId, sourceLanguage: d.sourceLanguage, targetLanguage: d.targetLanguage, position: Number.MAX_SAFE_INTEGER, flag: null, instructions: null, createdAt: '', goals: null })
      }
    }
    if (pairOrder) {
      result.sort((a, b) => {
            ...
            {allPairs.map(p => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    className="absolute top-0 right-0 w-10 h-10 flex items-center justify-center opacity-0 group-hover:opacity-100 text-ink-faint hover:text-ink transition-opacity text-base sm:text-2xl leading-none"
                    onClick={e => {
