...
export function FlashcardMode({ card, promptSide, promptLanguage, deckName, onRate, onAlmost, onPromptEdit, onAnswerEdit, onInfo, onToggleStar, hintable, onHint, answerLanguage, autoPlayAudio = true, ipaText, onToggleIPA, resumeAnswered = false }: {
  ...
  function reveal(asDontKnow = false) {
    ...
    setRevealed(true)
    if (autoPlayAudio && answerIsTarget && answerLanguage) speakCard(card, answerLanguage)
  }
      ...
      {!revealed ? (
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => reveal()} className="btn-primary px-10">Show answer</button>
          </div>
          <button onClick={() => reveal(true)} className="text-xs text-ink-faint hover:text-ink-muted">Don&apos;t know</button>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="panel relative min-h-[100px] flex items-center justify-center text-center bg-surface-raised">
            {onAnswerEdit
            ...
            {answerIsTarget && answerLanguage && (
              <button onClick={() => speakCard(card, answerLanguage)} title="Listen"
          ...
          {dontKnow ? (
            ...
            <div className="space-y-6">
              ...
              {/* "Almost" is deliberately NOT a rating — it's a near-miss escape hatch (light penalty +
                  ...
                  than in the rating row. The container is space-y-6 rather than the usual 3 so the gap
                  above it is wide enough that it doesn't read as a fifth rating button. */}
              {onAlmost && (
                <div className="flex justify-center">
