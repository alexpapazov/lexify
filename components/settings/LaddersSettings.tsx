...
function LaddersInner() {
  ...
  return (
    <div className="space-y-5">
      <div>
        {/* Only shown on a per-language page — it steps back up to the default ladder, which is real
            navigation. The Settings shell provides the way out of the section itself. */}
        {isPair && (
          <a href="/settings?section=ladders" className="inline-block text-xs text-ink-faint hover:text-ink mb-3">
            ← All learning ladders
          </a>
        )}
      ...
      {!isPair && pairs.length > 0 && (
        <div className="space-y-2 pt-2">
          ...
          {pairs.map(p => (
            <a key={`${p.source}|${p.target}`} href={`/settings?section=ladders&source=${p.source}&target=${p.target}`}
...
/**
 * The Learning ladders / pathways editor, rendered inside the Settings shell (`?section=ladders`).
 * Reads `?source=&target=` for the per-language editor, so it keeps its own Suspense boundary.
 */
export function LaddersSettings() {
  return <Suspense fallback={<p className="text-sm text-ink-faint">Loading…</p>}><LaddersInner /></Suspense>
}
