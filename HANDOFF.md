...
- **Backend**: Supabase (Postgres + Auth + RLS). Migrations `001`–`118`, applied BY HAND. **Nothing
  is pending** — the top level of `supabase/migrations/` is empty.
...
- **Migrations are applied by hand** in the Supabase SQL editor. Numbering is sequential.
  ...
  move it into `archive/` once it's live. **Nothing needs running.** Next number = **119**.
