...
export class ActiveTimer {
  ...
  /** Fold the time since the last interaction into the active total, capping a long idle gap at 40s. */
  tick() {
    const now = Date.now()
    this.active += Math.min(now - this.last, IDLE_CAP_MS)
    this.last = now
  }
...
export function useActiveTimer() {
  ...
  if (ref.current === null) ref.current = new ActiveTimer()
  useEffect(() => {
    const t = ref.current!
    const tick = () => t.tick()
