...
describe('forwardProductionMode — effective interval on the smart lane', () => {
  ...
  it('the typed lane always types; accelerated-confirmed always self-grades', () => {
    expect(forwardProductionMode({ ...base, intervalDays: 40 }, 'typed', 20)).toBe('typed')
    expect(forwardProductionMode({ ...base, acceleratedMode: 'import_known', acceleratedTypedConfirmed: true, intervalDays: 1 }, 'smart', 20)).toBe('self-graded')
  })
})
