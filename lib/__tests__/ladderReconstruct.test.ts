...
const rec = (over: Partial<ClimbRecord>): ClimbRecord => ({
  ...
  graduatedAtMs: new Date('2026-07-18T05:30:00Z').getTime(), ...over,
})
...
describe('reconstructEvents', () => {
  ...
  it('clusters same-pair graduations within the gap and splits far-apart ones', () => {
    const near = rec({ cardId: 'a', graduatedAtMs: new Date('2026-07-18T05:00:00Z').getTime() })
    const near2 = rec({ cardId: 'b', graduatedAtMs: new Date('2026-07-18T05:05:00Z').getTime() })
    const far = rec({ cardId: 'c', graduatedAtMs: new Date('2026-07-18T09:00:00Z').getTime() })
