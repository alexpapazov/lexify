...
describe('isCarryingDebt', () => {
  it('is false for a card sitting exactly where its own schedule put it', () => {
    ...
    expect(isCarryingDebt(grad(), '2026-08-31T04:00:00.000Z')).toBe(false)
  })
  ...
  it('is false for a card nudged within the fuzz window by Redistribute', () => {
    ...
    expect(isCarryingDebt(grad(), '2026-09-01T04:00:00.000Z')).toBe(false)   // gap 31 vs 30 × 1.05 + 1
  })
  ...
  it('is true for a card pushed well past its own interval without a review', () => {
    ...
    expect(isCarryingDebt(grad(), '2026-08-27T04:00:00.000Z')).toBe(false)
    expect(isCarryingDebt(grad({ lastReviewedAt: '2026-02-01T04:00:00.000Z' }), '2026-08-27T04:00:00.000Z')).toBe(true)
  })
  ...
  it('catches a card that was only mildly overdue before being dealt', () => {
    ...
    expect(isCarryingDebt(s, '2026-08-27T04:00:00.000Z')).toBe(true)
  })
  ...
  it('is false when there is no anchor to measure from at all', () => {
    expect(isCarryingDebt(grad({ lastReviewedAt: null, graduatedAt: null }), '2026-08-27T04:00:00.000Z')).toBe(false)
  })

  it('is false for a missing due date', () => {
    expect(isCarryingDebt(grad(), null)).toBe(false)
  })
})

describe('trackDueDates', () => {
  it('reads the production lane and recall separately on a forward row', () => {
    const s = grad({ smartDueAt: 'A', recallDueAt: 'B', dueAt: 'C' })
    expect(trackDueDates(s)).toEqual(['A', 'B'])
  })
  it('reads only the recall column on a reverse row', () => {
    expect(trackDueDates(grad({ reviewDirection: 'reverse', recallDueAt: 'R', dueAt: 'C' }))).toEqual(['R'])
  })
})
