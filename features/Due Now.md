...
>
> **Cloze is also a learning-ladder/pathway option (2026-09-07; + dictation 2026-09-10):**
...
| 2026-08-05 | No Listen (audio) button on Due Now self-graded reverse-recall cards — `FlashcardMode` only renders the prompt-panel Listen button (and autoplays) when `promptSide === 'front' && promptLanguage`, but none of the three session pages passed `promptLanguage` to the post-graduation `FlashcardMode` (nor to the Undo re-rate view) | Pass `promptLanguage={reviewPromptSide === 'front' ? sourceLanguage : undefined}` at all six FlashcardMode call sites (Due Now + re-rate × 3 pages), plus the page's standard `autoPlayAudio` gate on the Due Now ones (re-rate keeps `autoPlayAudio={false}`) |
