...
| Study sessions | `category=starred` (2026-08-10), SPLIT BY PHASE because a star cuts across graduation states and the two halves have different engines: **non-graduated** starred cards go through `LadderStudy` (`category=starred` on the ladder pages) — the pair's CURRENT ladder/pathway — and **graduated** ones through the elective review sessions (deck / folder / all), which take `category=starred` but only match graduated states. Each starred filter list shows up to two buttons: "Study Starred (n)" → ladder, "Review Starred (m)" → review. Neither is capped. **Never route a non-graduated card through the review pages' elective branches** — those build legacy step-pipeline queues (`initialCardState` + steps) and ignore the configured ladder; that was the reported "Step 1 · recognition ghost" bug |
