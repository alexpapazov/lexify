...
| 2026-07-02 | Synonym-group cards stuck in the pipeline (multi-field typing). At a `requiredCorrect=2` step (e.g. step 2) each member was marked "answered today" in the **day-persistent** prefill store after just **one** correct pass. Once every member was marked, `SynonymTypingMode` auto-skipped the group (`onAdvance([])`) for the rest of the day, and the group was only re-queued when a member *advanced a step* — which never happens on the first pass. Result: the group went dormant mid-step and never reappeared until the next day. Made worse by a mixed round (one form right, one wrong). | In `study/[deckId]/session/page.tsx` (only page with synonym multi-field typing): pipeline synonym grey-out is now **session-scoped** (`sessionAnsweredSynonyms`), not day-persistent — the day store (`synonymPrefill.ts`) is reserved for Due Now. `handleSynonymTypingAdvance` now (a) greys only session-scoped on a partial round, (b) clears all group marks when a full "all forms correct" milestone is reached so the next required-correct round re-prompts every form, and (c) **always re-queues the group** (rep's latest state) while the rep is still in-pipeline, so the reps/steps finish in one session. The auto-advance effect only skips a synonym rep when *every* group member is answered (session-scoped for pipeline; day-or-session for graduated). Added `unmarkSynonymAnswered()` to purge stale day marks. |
